...
describe VCAP::Services::Ping::Node do
  ...
  before :all do
    ...
    EM.run do
      @node = VCAP::Services::Ping::Node.new(@options)
      EM.add_timer(0.1) {
		@logger.warn("EM STOP HERE")
		EM.stop
	}
    end
  end
...
=end
  ...
  describe "Node.provision" do
    ...
    it "should return the url by successful provision" do
      url = get_connect_info(@p_service)
      @logger.info("Credentials: URL = #{url}")
      url.should be
